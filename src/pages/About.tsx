    ...
    <>
      ...
      <div className="min-h-svh bg-gradient-to-b from-[#f5f9fc] to-white py-10">
        <div className="mx-[10%] px-4">
          <h1 className="text-4xl font-bold text-center mb-8 text-[#1a365d]">Mon Parcours Professionnel</h1>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {experiences.map((exp, idx) => (
              <div key={idx} className="bg-white rounded-xl shadow-md p-6 flex flex-col md:flex-row items-center gap-6 border border-[#e3eaf3]">
                <img src={exp.logo} alt={exp.company + ' logo'} className="w-16 h-16 object-contain rounded-full border border-[#e3eaf3] bg-[#f5f9fc]" />
                <div className="flex-1">
                  <h2 className="text-2xl font-semibold text-[#1a365d]">{exp.title}</h2>
                  <p className="text-md text-[#3b5b7e] font-medium">{exp.company} {exp.type && `· ${exp.type}`}</p>
                  <p className="text-sm text-[#6b7a90]">{exp.date} {exp.location && `· ${exp.location}`}</p>
                  <ul className="list-disc pl-5 mt-2 text-[#1a365d]">
